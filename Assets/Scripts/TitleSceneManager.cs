        ...
        switch (_state)
        {
            case State.phase1:
                ...
                foreach (string command in commands)
                {
                    ...
                    if(luckOutput && dump_text == "{幸運}"){ dump_text = _playerJson.data.status[3].value.ToString(); }
                    ...
                    {
                        _player.skills.Add(new Skill(command, "回避", int.Parse(dump_text), AudioManager.Move.None));
                    }
                    else
                    {
                ...
                if (isHeal)
                {
                    switch (pieceType)
                    {
                        ...
                        case GameManager.PieceType.luck_CC:
                            _player.skills.Add(new Heal("CC<=30【応急手当】", "応急手当", 30, AudioManager.Move.FirstAid, 3));
                            break;
                        case GameManager.PieceType.Noluck_CCB:
                        case GameManager.PieceType.luck_CCB:
                            _player.skills.Add(new Heal("CCB<=30【応急手当】", "応急手当", 30, AudioManager.Move.FirstAid, 3));
                            break;
                    }
                }
            ...
            case State.phase2:
                ...
                foreach(GameObject obj in _atkMethodList)
                {
                    ...
                    Debug.LogFormat("actionName：{0}\nname：{1}",_dumpText, _dumpText_dp1);
